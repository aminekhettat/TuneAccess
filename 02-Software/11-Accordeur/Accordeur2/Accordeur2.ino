...
// ===============================
//   HAPTICS (pilotage du code 3 conservé)
// ===============================
...
static const uint32_t HAPTIC_DSP_MUTE_MS = 80; // plus court pour meilleure réactivité

// Anti-burst soft (évite spam I2C/standby)
...
// ====== VERSION LINEAIRE DU DUTY (ce que tu voulais) ======
...
static void calibrateNoiseFloor() {
  const int frames = 12; // réduit pour démarrage plus rapide
  double acc = 0;
...
static void TaskAudio(void *pv) {
  ...
  // lissage cents
  const float ALPHA = 0.18f;          // un peu plus réactif
  const float MAX_STEP_CENTS = 3.5f;  // plus réactif
  ...
  for (;;) {
    ...
    // mute DSP juste après vibration
    ...
    // Validation multi-trames (plus rapide)
    {
      float relTol = (profilActuel == CHROMATIQUE) ? 0.025f : 0.04f;
      uint8_t confirmNeeded = (profilActuel == CHROMATIQUE) ? 3 : 2; // réduit
    ...
    // Pipeline note dominante + cents median + EMA/slew
