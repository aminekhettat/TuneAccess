...
// Gate adaptatif dynamique (ajout code 1)
static const float SILENCE_RMS_THRES = 0.0012f;
static float g_noiseFloor = 0.0f;      // calibration boot
static float g_gateRms    = SILENCE_RMS_THRES; // base calibrée
...
// ===============================
//   Pitch tracking robustesse (ajout code 1)
// ===============================
...
// Ajout code 1 : mute DSP après vibration
...
// ===============================
//   Anti-secteur 50 Hz (ajout code 1)
// ===============================
static const float MAINS_REJECT_FMIN = 47.0f;
static const float MAINS_REJECT_FMAX = 53.0f;
static const float MAINS_REJECT_MAX_EXCESS = 0.0030f; // à ajuster selon ton micro I2S

// ===============================
//   Corde guitare (correction octave, ajout code 1)
// ===============================
...
static bool isFrequencyPlausible(float f, Instrument p) {
  ...
  if (p == GUITARE) return ratio <= 2.2f; // plus tolérant
  return ratio <= 1.6f;
}
...
static void hapticsHardOff() {
  ...
  // évite d'écrire standby en boucle à chaque frame
  uint32_t now = millis();
...
static void hapticsOn(uint8_t duty) {
  ...
  // sortir standby seulement si nécessaire (anti-burst I2C)
  ...
  // Ajout code 1 : mute DSP court après excitation haptique
  g_hapticMuteUntilMs = now + HAPTIC_DSP_MUTE_MS;
}

// Loi haptique “produit” + hystérésis seuil (gardée du code 3)
...
// Gate adaptatif dynamique (inspiré code 1)
static bool computeAdaptiveGate(float rms, float &gateUsed) {
  if (!g_noiseEmaInit) {
    g_noiseEma = g_gateRms; // base issue du calibrage
    g_noiseEmaInit = true;
  } else {
  ...
  if (gate > 0.02f) gate = 0.02f; // borne haute sécurité (à ajuster si besoin)
...
static void changerProfil(Instrument p) {
  ...
  // Reset tracking robustesse (ajouts code 1)
  resetDetectionState();
...
static float detectPitchYIN(const float *input, float *outYinMin = nullptr) {
  ...
  // petit heuristique harmonique (déjà présent dans ton code 3)
  for (int k = 2; k <= 4; k++) {
...
static void TaskAudio(void *pv) {
  ...
  for (;;) {
    ...
    // Gate adaptatif dynamique + hystérésis
    ...
    // Si trou de détection trop long, reset tracking
    ...
    // Validation multi-trames (inspirée code 1)
    {
    ...
    // Correction octave guitare (ajout code 1)
    if (profilActuel == GUITARE) {
      f = correctOctaveForGuitar(f);
    }

    // Rejet secteur 50 Hz (mode chromatique)
    ...
    // Rejet fréquence implausible (ajout code 1)
    ...
    // Lissage fréquence robuste (ajout code 1)
    ...
    // === Pipeline "note dominante + cents median + EMA/slew" (ton code 3) ===
