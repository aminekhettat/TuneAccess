...
// !!! IMPORTANT : on libère GPIO26 pour I2S audio OUT
// -> on déplace le PWM haptique sur une autre GPIO (ex: 16)
...
// Simulation batterie (ajout)
#if BATTERY_TEST_SERIAL
static volatile bool  g_battSimMode   = false;
static volatile float g_battSimSoc    = 50.0f;
static volatile float g_battSimVbat   = 3.90f;
static volatile bool  g_battForceRead = false;
#endif
...
static inline void pwmAttach(int pin, int freq, int res) {
  // Arduino-ESP32 core récent : ledcAttach(pin,freq,res)
...
static inline void setBatteryColor(float soc) {
  if ((!g_battOk && !g_battSimMode) || isnan(soc)) { setBattRGB(255, 0, 255); return; }
...
// ===============================
//   AUDIO I2S (MIC RX + SPK TX)
// ===============================
static const i2s_port_t I2S_PORT   = I2S_NUM_0;
static const int I2S_BCK_PIN       = 5;
static const int I2S_WS_PIN        = 25;
static const int I2S_DATA_PIN      = 35; // MIC IN
static const int I2S_DOUT_PIN      = 26; // SPK OUT (data_out)
...
// Mute DSP après vibration (réduit auto-perturbation)
static volatile uint32_t g_hapticMuteUntilMs = 0;
static const uint32_t HAPTIC_DSP_MUTE_MS = 80;
...
// ===============================
//   SPEAKER AMP (comme ton test)
// ===============================
static const gpio_num_t SPK_PW   = GPIO_NUM_21;
static const gpio_num_t SPK_GAIN = GPIO_NUM_23;

static void speakerAmpInit() {
  gpio_set_direction(SPK_PW, GPIO_MODE_OUTPUT);
  gpio_set_level(SPK_PW, 1);

  gpio_set_direction(SPK_GAIN, GPIO_MODE_OUTPUT);
  gpio_set_level(SPK_GAIN, 0); // adapte si besoin (0/1)
}

// ===============================
//   AUDIO FEEDBACK I2S (beeps)
// ===============================
static float g_beepPhase = 0.0f;

static void i2sWriteSilenceMs(int durationMs) {
  const int N = 256;
  int32_t buf[N] = {0};
  size_t bytesWritten = 0;
  int totalSamples = (int)(SAMPLE_RATE * durationMs / 1000.0f);

  for (int s = 0; s < totalSamples; s += N) {
    i2s_write(I2S_PORT, buf, sizeof(buf), &bytesWritten, portMAX_DELAY);
  }
}

static void i2sWriteBeepMs(float freq, int durationMs, float amp = 0.12f) {
  const int N = 256;
  int32_t buf[N];
  size_t bytesWritten = 0;
  ...
  for (int s = 0; s < totalSamples; s += N) {
    for (int i = 0; i < N; i++) {
      float x = sinf(g_beepPhase) * amp;
      g_beepPhase += w;
      if (g_beepPhase > 2.0f * PI) g_beepPhase -= 2.0f * PI;
...
static const uint32_t AUDIO_PITCH_TIMEOUT_MS = 250;

// plus c'est faux, plus c'est lent ; proche => rapide
static void TaskAudioFeedback(void *pv) {
  (void)pv;

  for (;;) {
    bool hasPitch = g_hasPitch && ((millis() - (uint32_t)g_lastPitchMs) <= AUDIO_PITCH_TIMEOUT_MS);

    if (!hasPitch) {
      i2sWriteSilenceMs(80);
    ...
    // zone OK
    if (a < 5.0f) {
      i2sWriteBeepMs(1200.0f, 60);
      i2sWriteSilenceMs(40);
      i2sWriteBeepMs(1200.0f, 60);
      i2sWriteSilenceMs(220);
      continue;
    }

    if (a > 50.0f) a = 50.0f;
    float t = (a - 5.0f) / (50.0f - 5.0f); // 0..1

    int periodMs = (int)(700 - t * (700 - 140));
    int beepMs   = 80;
    if (periodMs < beepMs + 10) periodMs = beepMs + 10;

    float f = (c < 0) ? 600.0f : 900.0f;

    i2sWriteBeepMs(f, beepMs);
    i2sWriteSilenceMs(periodMs - beepMs);
  }
}
...
static void updateHapticsFromCents(float cents) {
  ...
  // Normalisation linéaire [deadband..max]
  float x = 0.0f;
...
// ===============================
//   I2S init/read (FULL DUPLEX)
// ===============================
static bool initI2S() {
  i2s_config_t cfg = {};
  cfg.mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX | I2S_MODE_TX);
  cfg.sample_rate = (int)SAMPLE_RATE;              // 20000
  cfg.bits_per_sample = I2S_BITS_PER_SAMPLE_32BIT; // cohérent avec ton RX
  ...
  i2s_pin_config_t pins = {
    ...
    .data_out_num = I2S_DOUT_PIN,  // SPK OUT
    .data_in_num  = I2S_DATA_PIN   // MIC IN
  };
...
// ===============================
//   Commandes série batterie (simulation)
// ===============================
#if BATTERY_TEST_SERIAL
static void printBatteryTestHelp() {
  DBG_PRINTLN("\n[BAT TEST] Commandes:");
...
static void handleBatterySerialCommands() {
  ...
  if (lower == "bat real") {
    g_battSimMode = false;
    g_battForceRead = true;
    DBG_PRINTLN("[BAT] Retour mode capteur reel");
    return;
  }
  ...
  if (lower == "bat show") {
    DBG_PRINTF("[BAT] Mode=%s | VBAT=%.3f V | SOC=%.1f %%\n",
...
#if BATTERY_TEST_SERIAL
    if (g_battForceRead) {
      g_battForceRead = false;
      forceRead = true;
    }
#endif

    uint32_t now = millis();
    bool periodicRead = ((int32_t)(now - nextReadMs) >= 0);

    if (forceRead || periodicRead) {
      float v = NAN, s = NAN;
      bool ok = false;
...
static void TaskAudio(void *pv) {
  ...
  const float MAX_STEP_CENTS = 3.5f;
  ...
  for (;;) {
    ...
    const float gateOff = gateUsed * 0.75f;
...
void setup() {
  ...
  if (!g_battOk || !ok) {
    DBG_PRINTLN("BATTERIE: lecture echec (mode SIM possible).");
    setBatteryColor(NAN);
  } else {
  ...
  // Audio feedback sur l'autre core (ou core0)
  xTaskCreatePinnedToCore(TaskAudioFeedback, "AudioFB", 4096, nullptr, 1, nullptr, 0);
